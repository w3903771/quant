...
└── guide/               # 【产物二】可运行的 Python 教学指南  → 发布为 /guide/
    ...
    ├── js/mathjax.js    # 数学渲染配置
    ├── code/            # 完整脚本：每章独立子目录 code/chXX/<slug>.py（随站点发布，可在线查看/下载）
    ├── img/             # 教学图：每章独立子目录 img/chXX/*.png（随脚本生成，随站点发布）
    └── data/            # 示例缓存数据 / 快照（gitignore，不发布）
```
...
> **里程碑 1 进行中**：①✅ **地基库联调冒烟测试完成**（vectorbt/pyqlib/quantstats/polars 主栈可用；alphalens 系与 pandas3 不兼容 → ch03 走「从零 + qlib 评估」；详见 PLAN §4.2）；②✅ **ch0F 金融地基已完成**（8 节详尽地基 + `code/ch0F/finance_basics.py` 已实跑✅ + 8 图；A股制度经 WebSearch 核验，references 追加 R69–R76；经 academic-skills 五人评审并据评审改进：术语首现解释/哲学思辨引子/读图引导/修正下行波动口径）；③ 下一步：指南 ch00（第一个策略）+ 综述 01（全景与简史）。

### 综述（survey/）
- ✅ 00 导读与全书地图〔经 academic-skills 五人评审并落实全部建议：修编号错配·补全书15章总览(难度/篇幅)与按背景/A股实盘路径·术语精修·哲学思辨升级·两 ASCII 图升级为矢量 PNG〕
- ⬜ 01 量化投资全景与简史
...
### 指南（guide/）
- ✅ ch0F 金融市场与工具基础（0基础地基·详尽）〔代码已实跑✅·8图·A股制度已核验·经 academic-skills 五人评审并据评审改进：补术语解释/哲学思辨/读图引导+修正下行波动口径〕
- ⬜ ch00 环境、数据与第一个策略
- ⬜ ch01 金融数据处理与可视化
- ⬜ ch02 收益、风险与组合理论
- ⬜ ch03 因子与多因子模型
